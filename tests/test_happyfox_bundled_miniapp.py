...
def test_product_normalizer_versions_telegram_webapp_launch() -> None:
    ...
    main_menu_patch = Path("scripts/apply_happyfox_main_menu.py").read_text(
        encoding="utf-8"
    )
    ...
    assert "HappyFox Telegram system menu is not configured for Mini App" in main_menu_patch


def test_normalized_runtime_versions_webapp_and_keeps_native_webapp_menu() -> None:
    keyboards = Path("bot/keyboards.py").read_text(encoding="utf-8")
    main = Path("bot/main.py").read_text(encoding="utf-8")
    helper = main.split("async def _set_webapp_chat_menu_button() -> None:", 1)[1]
    helper = helper.split("\nasync def ", 1)[0]
    dockerfile = Path("Dockerfile").read_text(encoding="utf-8")
    ...
    assert '"type": "commands"' not in helper
    assert "_mini_app_url_with_start_param()" in helper
    assert "reconcile_telegram_webapp_menu.py" in dockerfile
    assert "ensure_telegram_webhook.py" in dockerfile
