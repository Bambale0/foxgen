...
def test_happyfox_runtime_uses_configurable_public_dns() -> None:
    ...
    assert "HAPPYFOX_DNS_PRIMARY:-1.1.1.1" in compose
    assert "HAPPYFOX_DNS_SECONDARY:-8.8.8.8" in compose
...
def test_telegram_webhook_reconciliation_preserves_pending_updates() -> None:
    ...
    assert "MenuButtonWebApp" in script
    assert "MenuButtonDefault" in script
    assert "MenuButtonCommands" not in script
    assert "_mini_app_url_with_release" in script
