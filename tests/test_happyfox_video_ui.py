...
def test_incompatible_model_is_replaced_when_type_changes() -> None:
    assert compatible_video_model("text", "glow") == VIDEO_TYPE_ROWS["text"][0]
    assert compatible_video_model("imgtxt", "veo3") == VIDEO_TYPE_ROWS["imgtxt"][0]
    assert compatible_video_model("video", "v3_pro") == VIDEO_TYPE_ROWS["video"][0]
