...
def test_motion_control_validates_current_provider_limits_before_upload() -> None:
    script = (MINIAPP / "motion-control.js").read_text(encoding="utf-8")

    assert "10 * 1024 * 1024" in script
    assert "100 * 1024 * 1024" in script
    assert "const MIN_DURATION = 3" in script
    assert "const IMAGE_ORIENTATION_MAX_DURATION = 10" in script
    assert "const VIDEO_ORIENTATION_MAX_DURATION = 30" in script
    assert "const MIN_RATIO = 2 / 5" in script
    assert "const MAX_RATIO = 5 / 2" in script
    assert "validateImage(file)" in script
    assert "validateVideo(file, orientationValue)" in script
    assert "videoMetadata(file)" in script


def test_motion_control_exposes_current_quality_and_orientation_modes() -> None:
    script = (MINIAPP / "motion-control.js").read_text(encoding="utf-8")

    assert 'name="motion-mode"' in script
    assert 'value="720p"' in script
    assert 'value="1080p"' in script
    assert 'name="motion-orientation"' in script
    assert 'value="image"' in script
    assert 'value="video"' in script
    assert "mode: modeValue" in script
    assert "character_orientation: orientationValue" in script
    assert "background_source: 'input_video'" in script
