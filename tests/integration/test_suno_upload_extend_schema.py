...
@pytest.mark.asyncio
async def test_upload_extend_owner_guard_trigger_and_function_exist() -> None:
            ...
            trigger = await session.scalar(
                text(
                    ...
                    SELECT EXISTS (
                        SELECT 1
                        FROM pg_trigger
                        WHERE tgname = 'trg_generations_suno_upload_extend_input'
            ...
            function = await session.scalar(
                text(
                    ...
                    SELECT EXISTS (
                        SELECT 1
                        FROM pg_proc
                        WHERE proname = 'foxgen_validate_suno_upload_extend_input'
                    )
                    """
                )
            )
