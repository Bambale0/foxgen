...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("provider", sa.String(length=64), server_default="telegram_stars", nullable=False),
        ...
        sa.Column("raw_payment", postgresql.JSONB(astext_type=sa.Text()), server_default=sa.text("'{}'::jsonb"), nullable=False),
        ...
        sa.UniqueConstraint("user_id", "idempotency_key", name="uq_user_payment_orders_user_idempotency"),
        sa.UniqueConstraint("invoice_payload", name="uq_user_payment_orders_invoice_payload"),
        sa.UniqueConstraint("telegram_payment_charge_id", name="uq_user_payment_orders_telegram_charge"),
    )
