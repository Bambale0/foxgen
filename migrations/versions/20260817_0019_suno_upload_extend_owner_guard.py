"""Enforce owner-bound Suno Upload & Extend input references.
...
_OWNER_GUARD_FUNCTION = "foxgen_validate_suno_upload_extend_input"
_OWNER_GUARD_TRIGGER = "trg_generations_suno_upload_extend_input"
...
def upgrade() -> None:
    op.execute(
        ...
        DECLARE
            ...
            miniapp_prefix text;
        BEGIN
            IF NEW.model_slug <> 'suno-v5-upload-extend' THEN
                RETURN NEW;
            END IF;
            ...
            IF storage_key IS NULL
               ...
               OR NOT (
                ...
                RAISE EXCEPTION 'Suno Upload & Extend input is not owned by generation user'
                    USING ERRCODE = '23514',
                          CONSTRAINT = 'ck_generations_suno_upload_extend_owner_input';
            END IF;
