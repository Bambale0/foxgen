    ...
    try {
      ...
      await page.addInitScript(
        ({ initDataValue, bridgePlatform }) => {
          window.WebApp = {
            ...
            initDataUnsafe: {
              start_param: 'max_e2e_start',
            },
            platform: bridgePlatform,
            version: '26.20.0',
            ready() {
              window.__MAX_READY_CALLED__ = true
            },
            expand() {
              window.__MAX_EXPAND_CALLED__ = true
            },
            openLink() {},
          }
        },
      ...
      await page.route('https://st.max.ru/js/max-web-app.js', async (route) => {
        await route.fulfill({
          ...
          body: '// MAX Bridge is provided by the E2E init script.\n',
        })
      })
      ...
      assert.equal(
        bootstrapRequest.platform,
        'max',
        `${target.name}: frontend must identify MAX platform`,
      )
      assert.equal(
        bootstrapRequest.init_data,
        initData,
        `${target.name}: MAX initData must reach bootstrap`,
      )
      ...
      const state = await page.evaluate(() => ({
        ...
        scripts: Array.from(document.head.querySelectorAll('script')).map((script) => ({
      ...
      assert.equal(
        state.text.includes('Войдите через Telegram'),
        false,
