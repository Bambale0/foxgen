...
type Profile = { user_id: number; slug: string; display_name?: string | null; bio?: string | null }
...
export function ProfileTab() {
  ...
  useEffect(() => { void load().catch((reason) => setMessage(reason instanceof Error ? reason.message : String(reason))) }, [])
          ...
          {publications.map((item) => (
            <div className="work-card" key={item.id}>
              <div className="work-row">
                ...
                <div className="work-copy"><strong>{item.model_slug}</strong><small>{item.scope} · ♥ {item.likes_count}</small></div>
              </div>
              <div className="card-actions"><button onClick={() => void remove(item.generation_id, item.scope)}>Снять публикацию</button></div>
            </div>
          ))}
