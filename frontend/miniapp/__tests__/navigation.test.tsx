...
const { bootstrap, apiMock } = vi.hoisted(() => {
  const data = {
    brand: 'Happy Fox',
...
describe('Happy Fox primary navigation', () => {
  ...
  it('uses the backend bootstrap data rather than a hardcoded model list', async () => {
    renderApp()
    await screen.findByTestId('screen-home')
    expect(bootstrap.models[0].slug).toBe('gpt-image-2')
    expect(screen.getByText('GPT Image 2')).toBeInTheDocument()
  })
})
