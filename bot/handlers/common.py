...
def _feed_cache_remove_card(cache: dict, key, gen_id: int) -> None:
    cached = _feed_cache_get(cache, key)
    if not cached:
        return
    cards = cached[-1]
    filtered_cards = [item for item in cards if int(item.get("id") or 0) != gen_id]
    _feed_cache_put(cache, key, *cached[:-1], filtered_cards)


def _feed_cache_remove_all_sources(viewer_user_id: int | None, gen_id: int) -> None:
    viewer_ids = {None, viewer_user_id}
    for source_code in FEED_SOURCE_CODES:
        for cached_viewer_id in viewer_ids:
            _feed_cache_remove_card(_feed_cards_cache, (source_code, cached_viewer_id), gen_id)
...
def _feed_cache_patch_all_sources(viewer_user_id: int | None, updated_card: dict) -> None:
    ...
    for source_code in FEED_SOURCE_CODES:
        for cached_viewer_id in viewer_ids:
            _feed_cache_patch_card(_feed_cards_cache, (source_code, cached_viewer_id), updated_card)
...
def _remove_feed_card_caches(gen_id: int, viewer_user_id: int | None, profile_code: str | None = None) -> None:
    _feed_cache_remove_all_sources(viewer_user_id, gen_id)
    if profile_code:
        _feed_cache_remove_card(
            _profile_feed_cards_cache,
            _profile_feed_cache_key(profile_code, viewer_user_id),
            gen_id,
        )
...
async def _build_feed_keyboard(
    *,
    ...
    if gen_id and viewer_telegram_id:
        is_admin_viewer = config.is_admin(viewer_telegram_id)
        can_blur = is_admin_viewer or bool(card.get("is_mine"))
        admin_buttons: list[types.InlineKeyboardButton] = []
        if can_blur:
        ...
        if is_admin_viewer:
            delete_callback = (
                f"bfdel:{gen_id}:p:{profile_code}:{index}:{photo_index}"
                if profile_code
                else f"bfdel:{gen_id}:s:{source_code}:{index}:{photo_index}"
            )
            admin_buttons.append(
                types.InlineKeyboardButton(
                    text="🗑 Удалить",
...
async def _fetch_feed_cards(
    ...
    *,
    ...
    cache_key = _feed_source_cache_key(source_code, None)
...
@router.callback_query(F.data.startswith("bfdel:"))
async def delete_feed_card(callback: types.CallbackQuery):
    ...
    removed = await remove_from_feed(gen_id, viewer.id, allow_any_user=True)
    if not removed:
        await _safe_callback_answer(callback, "Пост не найден", show_alert=True)
        return

    await _safe_callback_answer(callback, "Удалено из ленты")
    if mode == "p":
        referral_code = parts[3] if len(parts) > 3 else ""
        _remove_feed_card_caches(gen_id, viewer.id, profile_code=referral_code)
        index = _parse_int(parts[4], 0) if len(parts) > 4 else 0
        photo_index = _parse_int(parts[5], 0) if len(parts) > 5 else 0
        await _render_profile_feed_deeplink(
            callback.message,
            viewer,
            referral_code,
        ...
        source_code = parts[3] if len(parts) > 3 and parts[3] in FEED_SOURCE_CODES else "r"
        _remove_feed_card_caches(gen_id, viewer.id)
        index = _parse_int(parts[4], 0) if len(parts) > 4 else 0
        photo_index = _parse_int(parts[5], 0) if len(parts) > 5 else 0
        await _render_feed_by_source(
            callback.message,
...
async def toggle_feed_blur(callback: types.CallbackQuery):
    ...
    viewer = await get_or_create_user(callback.from_user.id)
    is_admin_viewer = config.is_admin(callback.from_user.id)

    updated = await set_feed_blurred(
        gen_id,
        viewer.id,
        blurred,
        allow_any_user=is_admin_viewer,
    )
    if not updated:
        await _safe_callback_answer(callback, "Пост не найден или нет доступа", show_alert=True)
        return
    ...
    if mode == "p":
        ...
        _patch_like_caches(updated, viewer.id, profile_code=referral_code)
        ...
        _patch_like_caches(updated, viewer.id)
        index = _parse_int(parts[5], 0) if len(parts) > 5 else 0
        photo_index = _parse_int(parts[6], 0) if len(parts) > 6 else 0
        await _render_feed_by_source(
            callback.message,
