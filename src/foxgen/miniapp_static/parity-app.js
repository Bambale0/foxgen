...
function bindForms(){const comment=document.getElementById('comment-form');if(comment)comment.addEventListener('submit',async e=>{e.preventDefault();const body=new FormData(comment).get('body')?.toString().trim();if(!body)return;try{const c=await api(`/publications/${state.publication.id}/comments`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({surface:state.publication.scope,body})});state.comments.push(c);comment.reset();render();}catch(err){toast(err.message,'error');}});}
