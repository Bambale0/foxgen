...
SUNO_UPLOAD_COVER_MODEL_SLUG = "suno-v5-upload-cover"
...
class UploadCoverSubmissionService(Protocol):
    async def submit(
        self,
        *,
        user_id: int,
...
class SunoUploadCoverService:
    def __init__(
        self,
        *,
        input_media: InputMediaInspector,
        submission: UploadCoverSubmissionService,
        max_bytes: int,
    ) -> None:
    ...
    async def submit(
        ...
        *,
        ...
        normalized = validate_input(InputContract.SUNO_V5_UPLOAD_COVER, input_data)
        ...
        if not media.content_type.lower().startswith("audio/"):
            raise SubmissionError(
                ErrorCode.VALIDATION,
                "Для Suno Cover нужен аудиофайл.",
            )
...
def _owned_input_key(user_id: int, storage_key: str) -> bool:
    if user_id <= 0 or ".." in storage_key or "://" in storage_key:
        return False
    return storage_key.startswith(f"inputs/{user_id}/") or storage_key.startswith(
