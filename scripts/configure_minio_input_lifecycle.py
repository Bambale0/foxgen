...
def desired_rule(retention_days: int) -> dict[str, Any]:
    if retention_days <= 0:
...
def merge_lifecycle_rules(
    ...
    *,
    ...
    preserved.append(desired_rule(retention_days))
    return preserved


def lifecycle_rule_matches(rule: dict[str, Any], *, retention_days: int) -> bool:
    return (
        rule.get("ID") == RULE_ID
...
def configure_lifecycle(
    ...
    *,
    ...
    existing_rules = _read_rules(client, bucket=bucket)
    ...
    matching = [
        ...
        if lifecycle_rule_matches(rule, retention_days=retention_days)
    ]
    if len(matching) != 1:
        raise RuntimeError(
            "Input lifecycle verification failed: expected exactly one enabled "
            f"{INPUT_PREFIX!r} rule with {retention_days} retention days"
        )
