...
app_path = Path("src/foxgen/miniapp_static/parity-app.js")
...
replace_required(
    "async function ensureScreenData(screen){try{if(screen==='feed')await loadFeed(false);if(screen==='works')await loadWorks(false);if(screen==='profile')await loadOwnProfile();if(screen==='references')await loadReferences();if(screen==='wallet')await loadWallet();render();}catch(e){toast(e.message,'error');}}",
...
replace_required(
    "const screens={feed:renderFeed,create:renderCreate,studio:renderStudio,works:renderWorks,profile:renderProfile,wallet:renderWallet,references:renderReferencesScreen,publication:renderPublication,publicProfile:renderPublicProfile,generation:renderGeneration};",
...
replace_required(
    '<button disabled>Партнёры и рефералы <small>Backend epic ещё не завершён</small></button><button disabled>Поддержка <small>Backend flow ещё не завершён</small></button>',
    '<button data-nav="tariff">Тарифы <span>›</span></button><button data-nav="partner">Партнёры <span>›</span></button><button data-nav="support">Поддержка <span>›</span></button>',
)
...
'''
if "function renderTariff(){" not in text:
    anchor = "\nfunction renderFatal(error)"
    if anchor not in text:
        raise SystemExit("renderFatal anchor missing")
    text = text.replace(anchor, "\n" + portal_block + anchor, 1)

old_bind = "function bindForms(){const comment=document.getElementById('comment-form');if(comment)comment.addEventListener('submit',async e=>{e.preventDefault();const body=new FormData(comment).get('body')?.toString().trim();if(!body)return;try{const c=await api(`/publications/${state.publication.id}/comments`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({surface:state.publication.scope,body})});state.comments.push(c);comment.reset();render();}catch(err){toast(err.message,'error');}});}"
new_bind = r'''function bindForms(){
...
replace_required(old_bind, new_bind)

click_anchor = "    if(el.dataset.nav){nav(el.dataset.nav);return;}"
click_new = click_anchor + "\n    if(el.dataset.supportTicket){await loadSupportTicket(el.dataset.supportTicket);return;}\n    if(el.hasAttribute('data-refresh-support')){await loadSupport();render();return;}\n    if(el.dataset.closeTicket){if(await confirmAction('Закрыть это обращение?')){state.supportTicket=await api(`/support/${el.dataset.closeTicket}/close`,{method:'POST'});await loadSupport();render();toast('Обращение закрыто');}return;}\n    if(el.hasAttribute('data-partner-join')){await api('/partner/join',{method:'POST'});await loadPartner();render();toast('Партнёрская программа подключена');return;}"
replace_required(click_anchor, click_new)

app_path.write_text(text, encoding="utf-8")

css_path = Path("src/foxgen/miniapp_static/parity.css")
css = css_path.read_text(encoding="utf-8")
portal_css = r'''

/* User portal: forms stay clean; grunge remains decorative and restrained. */
.portal-card{background:#111;border:1px solid rgba(255,255,255,.08);border-radius:20px;padding:18px;margin:14px 0}.portal-card h1,.portal-card h2{margin:0 0 12px}.portal-kv{display:grid;gap:8px}.portal-kv>div{display:flex;justify-content:space-between;gap:16px;padding:11px 0;border-bottom:1px solid rgba(255,255,255,.07)}.portal-kv span{color:#999}.portal-kv strong{text-align:right;max-width:65%;overflow-wrap:anywhere}.portal-form{display:grid;gap:12px}.portal-form label{display:grid;gap:7px;color:#aaa;font-size:13px}.portal-form input,.portal-form textarea{width:100%;box-sizing:border-box;background:#090909;color:#f5f5f5;border:1px solid rgba(255,255,255,.12);border-radius:14px;padding:13px 14px;font:inherit;outline:none}.portal-form textarea{min-height:110px;resize:vertical}.portal-form input:focus,.portal-form textarea:focus{border-color:rgba(255,119,32,.72);box-shadow:0 0 0 3px rgba(255,119,32,.08)}.ticket-list{display:grid;gap:9px}.ticket-list>button{display:flex;width:100%;align-items:center;justify-content:space-between;text-align:left;background:#101010;color:#fff;border:1px solid rgba(255,255,255,.07);border-radius:16px;padding:14px}.ticket-list span{display:grid;gap:5px}.ticket-list small{color:#888}.support-thread{display:grid;gap:10px;margin:14px 0}.support-message{max-width:88%;border-radius:17px;padding:12px 14px;background:#151515;border:1px solid rgba(255,255,255,.07)}.support-message.mine{justify-self:end;background:rgba(255,111,23,.12);border-color:rgba(255,111,23,.28)}.support-message p{white-space:pre-wrap;margin:6px 0}.support-message small{color:#777}.portal-close{width:100%;margin-top:10px}.partner-stats{display:grid;grid-template-columns:1fr 1fr;gap:9px;margin:14px 0}.partner-stats>div{background:#111;border:1px solid rgba(255,255,255,.07);border-radius:17px;padding:14px;display:grid;gap:4px}.partner-stats small{color:#888}.partner-stats strong{font-size:20px}.withdraw-list{display:grid;gap:8px}.withdraw-list>div{display:flex;justify-content:space-between;gap:12px;padding:13px;background:#101010;border-radius:14px;border:1px solid rgba(255,255,255,.06)}.withdraw-list span{display:grid;gap:4px}.withdraw-list small{color:#888}.join-partner p{color:#aaa;line-height:1.5}.settings-list button:not(:disabled){cursor:pointer}
'''
if "/* User portal: forms stay clean" not in css:
    css += portal_css
css_path.write_text(css, encoding="utf-8")

test_path = Path("tests/test_miniapp_user_portal_frontend.py")
...
docs = Path("docs/miniapp.md")
doc = docs.read_text(encoding="utf-8")
if "## User portal parity\n" not in doc:
    section = '''## User portal parity\n\nHappy Fox exposes owner-scoped **Tariffs**, **Support** and **Partners** through the Telegram-derived Mini App JWT. Tariffs render only the current published server version. Support can create/list/read/reply/close the current user's durable tickets. Partners can join, inspect earned/pending/available units and referral count, review withdrawal history and create an owner-scoped withdrawal request.\n\nThese routes reuse the existing durable admin-domain tables but never expose admin HMAC credentials or operator actions to browser JavaScript.\n\n'''
    anchor = "## Paid generation boundary\n"
    if anchor not in doc:
        raise SystemExit("docs miniapp anchor missing")
    docs.write_text(doc.replace(anchor, section + anchor, 1), encoding="utf-8")
