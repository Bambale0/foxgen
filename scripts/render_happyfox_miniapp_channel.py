...
def render_html(html: str, channel: str) -> str:
    ...
    match = HEAD_RE.search(rendered)
    if match is None:
        raise ValueError("Mini App HTML does not contain <head>")
    rendered = rendered[: match.end()] + marker + rendered[match.end() :]

    telegram_count = len(TELEGRAM_SCRIPT_RE.findall(rendered))
    max_count = len(MAX_SCRIPT_RE.findall(rendered))
    expected = {
        "shared": (1, 1),
        "telegram": (1, 0),
        "max": (0, 1),
    }[channel]
    if (telegram_count, max_count) != expected:
        raise ValueError(
            "Mini App bridge contract mismatch: "
            f"channel={channel} telegram={telegram_count} max={max_count} expected={expected}"
        )
    return rendered
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Render a HappyFox Mini App static export for one messenger channel"
    )
