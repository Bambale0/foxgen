...
def _patch_generation() -> None:
    ...
    for callback_name in ("v_type_text", "v_type_imgtxt", "v_type_video"):
        block = text.split(f'@router.callback_query(F.data == "{callback_name}")', 1)[1]
        block = block.split("@router.callback_query", 1)[0]
        if "_show_video_media_screen" in block:
            raise RuntimeError(f"{callback_name} still returns to the legacy media wizard")
        if "_show_video_creation_screen" not in block:
            raise RuntimeError(f"{callback_name} does not redraw the one-screen video UI")
